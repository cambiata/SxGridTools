...
class ScorxDb {
	...
	static public function insertChannelData(filename:String, id:String, name:String, data:Bytes)
	{
		var cnx = Sqlite.open(filename);	
		var blobdata = Lib.stringReference(data);
		var cnx = Sqlite.open(filename);	
		var sql = 'INSERT INTO channels (id, name, data) VALUES (' + cnx.quote(id) +  ', ' + cnx.quote(name) + ', ' + cnx.quote(blobdata) + ')';
		var cnx = Sqlite.open(filename);	
		cnx.request(sql);
		cnx.close();		
	}
	
	static public function insertPagesData(filename:String, id:String,data:Bytes)
	{
		var cnx = Sqlite.open(filename);	
		var blobdata = Lib.stringReference(data);
		var cnx = Sqlite.open(filename);	
		var sql = 'INSERT INTO pages (id, data) VALUES (' + cnx.quote(id) + ', ' + cnx.quote(blobdata) + ')';
		var cnx = Sqlite.open(filename);	
		cnx.request(sql);
		cnx.close();		
	}	
