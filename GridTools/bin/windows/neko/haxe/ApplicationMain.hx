...
class ApplicationMain {
	...
	public static function main () {
		...
		flash.Lib.setPackage ("Jonas Nyström", "GridTools", "sx.GridTools", "1.0.0");
		...
		flash.Lib.create (function () {
				...
				for (methodName in Type.getClassFields (sx.Main)) {
				...
				if (hasMain) {
					...
					Reflect.callMethod (sx.Main, Reflect.field (sx.Main, "main"), []);
...
@:keep class DocumentClass extends sx.Main {}
...
class ApplicationMain {
	...
	public static function main () {
		...
		wx.App.boot (function () {
			...
			frame = wx.Frame.create (null, null, "GridTools", null, { width: 1200, height: 600 });
			
			
			#if openfl
			var stage = wx.NMEStage.create (frame, null, null, { width: 1200, height: 600 });
			#end
			...
			if (hasMain) {
				...
				var instance = Type.createInstance (sx.Main, []);
			}
