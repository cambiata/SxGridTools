...
Void VSlider_obj::_onMouseDown( ::flash::events::MouseEvent event){
{
...
Void VSlider_obj::_onScreenMouseMove( ::flash::events::MouseEvent event){
{
		...
		HX_STACK_ARG(event,"event")
		HX_STACK_LINE(25)
		Float ypos = ((event->stageY - this->get_stageY()) - this->_mouseDownOffset);		HX_STACK_VAR(ypos,"ypos");
		HX_STACK_LINE(26)
		this->set_pos(::Std_obj::_int(this->calcPosFromCoord(ypos)));
	}
return null();
}


Void VSlider_obj::_onBackgroundMouseDown( ::flash::events::MouseEvent event){
{
		...
		if (((this->_thumb->hitTest(event->stageX,event->stageY) == false))){
			HX_STACK_LINE(31)
			Float ypos = (event->stageY - this->get_stageY());		HX_STACK_VAR(ypos,"ypos");
			HX_STACK_LINE(32)
			hx::SubEq(ypos,(Float(this->_thumb->get_height()) / Float((int)2)));
			HX_STACK_LINE(33)
			this->set_pos(::Std_obj::_int(this->calcPosFromCoord(ypos)));
			HX_STACK_LINE(34)
			this->_thumb->set_state(HX_CSTRING("down"));
			HX_STACK_LINE(35)
			this->startTracking((Float(this->_thumb->get_height()) / Float((int)2)));
		}
	}
return null();
}
...
Float VSlider_obj::calcPosFromCoord( Float ypos){
	...
	HX_STACK_LINE(54)
	Float maxY = (_g - this->_thumb->get_height());		HX_STACK_VAR(maxY,"maxY");
	HX_STACK_LINE(56)
	if (((ypos < minY))){
		HX_STACK_LINE(57)
		ypos = minY;
	...
	HX_STACK_LINE(64)
	Float _g1 = this->get_max();		HX_STACK_VAR(_g1,"_g1");
	HX_STACK_LINE(64)
	int m = ::Std_obj::_int((_g1 - this->get_min()));		HX_STACK_VAR(m,"m");
	HX_STACK_LINE(65)
	Float v = (ypos - minY);		HX_STACK_VAR(v,"v");
	HX_STACK_LINE(66)
	Float newValue = (this->get_max() - ((Float(v) / Float(ucy)) * m));		HX_STACK_VAR(newValue,"newValue");
	HX_STACK_LINE(67)
	return newValue;
}
