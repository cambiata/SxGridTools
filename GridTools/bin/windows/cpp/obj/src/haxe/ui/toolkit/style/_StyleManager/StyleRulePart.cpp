...
{
HX_STACK_FRAME("haxe.ui.toolkit.style._StyleManager.StyleRulePart","new",0x00e30c00,"haxe.ui.toolkit.style._StyleManager.StyleRulePart.new","haxe/ui/toolkit/style/StyleManager.hx",379,0x60b8f881)
...
HX_STACK_ARG(rulePart,"rulePart")
{
	...
	if (((n != (int)-1))){
		HX_STACK_LINE(382)
		this->state = rulePart.substr((n + (int)1),rulePart.length);
		HX_STACK_LINE(383)
		rulePart = rulePart.substr((int)0,n);
	}
	HX_STACK_LINE(386)
	if ((::StringTools_obj::startsWith(rulePart,HX_CSTRING("#")))){
		HX_STACK_LINE(387)
		this->id = rulePart.substr((int)1,rulePart.length);
		HX_STACK_LINE(388)
		n = this->id.indexOf(HX_CSTRING("."),null());
		HX_STACK_LINE(389)
		if (((n != (int)-1))){
			HX_STACK_LINE(390)
			this->styleName = this->id.substr((n + (int)1),this->id.length);
			HX_STACK_LINE(391)
			this->id = this->id.substr((int)0,n);
		}
	}
	else{
		HX_STACK_LINE(394)
		...
		HX_STACK_LINE(395)
		n = this->className.indexOf(HX_CSTRING("."),null());
		HX_STACK_LINE(396)
		if (((n != (int)-1))){
			HX_STACK_LINE(397)
			this->styleName = this->className.substr((n + (int)1),this->className.length);
			HX_STACK_LINE(398)
			this->className = this->className.substr((int)0,n);
			HX_STACK_LINE(399)
