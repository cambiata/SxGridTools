...
Void Image_obj::initialize( ){
{
		...
		if (((this->_bmp != null()))){
			...
			HX_STACK_LINE(33)
			if (((_g > this->_bmp->get_height()))){
				HX_STACK_LINE(34)
				Float _g1 = (Float(this->get_height()) / Float((int)2));		HX_STACK_VAR(_g1,"_g1");
				HX_STACK_LINE(34)
				this->_bmp->set_y(::Std_obj::_int((_g1 - (Float(this->_bmp->get_height()) / Float((int)2)))));
			}
		}
	}
return null();
...
Dynamic Image_obj::set_resource( Dynamic value){
	...
	HX_STACK_LINE(99)
	::flash::display::BitmapData bmpData = null();		HX_STACK_VAR(bmpData,"bmpData");
	HX_STACK_LINE(100)
	if ((::Std_obj::is(value,hx::ClassOf< ::String >()))){
		HX_STACK_LINE(101)
		bmpData = ::haxe::ui::toolkit::resources::ResourceManager_obj::get_instance()->getBitmapData(hx::TCast< String >::cast(value),null());
	}
	else{
		HX_STACK_LINE(102)
		if ((::Std_obj::is(value,hx::ClassOf< ::flash::display::Bitmap >()))){
			HX_STACK_LINE(103)
			bmpData = (hx::TCast< flash::display::Bitmap >::cast(value))->bitmapData;
		}
		else{
			HX_STACK_LINE(104)
			if ((::Std_obj::is(value,hx::ClassOf< ::flash::display::BitmapData >()))){
				HX_STACK_LINE(105)
				bmpData = hx::TCast< flash::display::BitmapData >::cast(value);
			}
		}
	}
	HX_STACK_LINE(108)
	if (((bmpData != null()))){
		HX_STACK_LINE(109)
		this->_bmp = ::flash::display::Bitmap_obj::__new(bmpData,null(),null());
		HX_STACK_LINE(110)
		this->get_sprite()->addChild(this->_bmp);
		HX_STACK_LINE(111)
		if (((this->get_autoSize() == true))){
			HX_STACK_LINE(112)
			this->set_width(bmpData->get_width());
			HX_STACK_LINE(113)
			this->set_height(bmpData->get_height());
		}
	}
	HX_STACK_LINE(117)
...
Dynamic Image_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 4:
		if (HX_FIELD_EQ(inName,"_bmp") ) { _bmp=inValue.Cast< ::flash::display::Bitmap >(); return inValue; }
		break;
	case 7:
...
static hx::StorageInfo sMemberStorageInfo[] = {
	{hx::fsObject /*::flash::display::Bitmap*/ ,(int)offsetof(Image_obj,_bmp),HX_CSTRING("_bmp")},
