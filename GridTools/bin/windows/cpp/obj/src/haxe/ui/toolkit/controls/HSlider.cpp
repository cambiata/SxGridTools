...
Void HSlider_obj::_onMouseDown( ::flash::events::MouseEvent event){
{
...
Void HSlider_obj::_onScreenMouseMove( ::flash::events::MouseEvent event){
{
		...
		HX_STACK_ARG(event,"event")
		HX_STACK_LINE(25)
		Float xpos = ((event->stageX - this->get_stageX()) - this->_mouseDownOffset);		HX_STACK_VAR(xpos,"xpos");
		HX_STACK_LINE(26)
		this->set_pos(::Std_obj::_int(this->calcPosFromCoord(xpos)));
	}
return null();
}


Void HSlider_obj::_onBackgroundMouseDown( ::flash::events::MouseEvent event){
{
		...
		if (((this->_thumb->hitTest(event->stageX,event->stageY) == false))){
			HX_STACK_LINE(31)
			Float xpos = (event->stageX - this->get_stageX());		HX_STACK_VAR(xpos,"xpos");
			HX_STACK_LINE(32)
			hx::SubEq(xpos,(Float(this->_thumb->get_width()) / Float((int)2)));
			HX_STACK_LINE(33)
			this->set_pos(::Std_obj::_int(this->calcPosFromCoord(xpos)));
			HX_STACK_LINE(34)
			this->_thumb->set_state(HX_CSTRING("down"));
			HX_STACK_LINE(35)
			this->startTracking((Float(this->_thumb->get_width()) / Float((int)2)));
		}
	}
return null();
}
...
Float HSlider_obj::calcPosFromCoord( Float xpos){
	...
	HX_STACK_LINE(54)
	Float maxX = (_g - this->_thumb->get_width());		HX_STACK_VAR(maxX,"maxX");
	HX_STACK_LINE(56)
	if (((xpos < minX))){
		HX_STACK_LINE(57)
		xpos = minX;
