...
Void CalendarLayout_obj::resizeChildren( ){
{
		...
		HX_STACK_LINE(167)
		Float _g1 = (_g - ((int)6 * this->get_spacingX()));		HX_STACK_VAR(_g1,"_g1");
		HX_STACK_LINE(167)
		Float _g2 = this->get_padding()->get_left();		HX_STACK_VAR(_g2,"_g2");
		HX_STACK_LINE(167)
		Float ucx = (_g1 + ((_g2 + this->get_padding()->get_right())));		HX_STACK_VAR(ucx,"ucx");
		HX_STACK_LINE(168)
		Float _g3 = this->get_usableHeight();		HX_STACK_VAR(_g3,"_g3");
		HX_STACK_LINE(168)
		Float _g4 = (_g3 - ((int)5 * this->get_spacingY()));		HX_STACK_VAR(_g4,"_g4");
		HX_STACK_LINE(168)
		Float _g5 = this->get_padding()->get_top();		HX_STACK_VAR(_g5,"_g5");
		HX_STACK_LINE(168)
		Float ucy = (_g4 + ((_g5 + this->get_padding()->get_bottom())));		HX_STACK_VAR(ucy,"ucy");
		HX_STACK_LINE(169)
		...
		HX_STACK_LINE(175)
		{
			...
			while(((_g6 < (int)6))){
				...
				HX_STACK_LINE(176)
				{
					...
					while(((_g11 < (int)7))){
						...
						if (((child != null()))){
							...
							(n)++;
							HX_STACK_LINE(185)
							hx::AddEq(xpos,(bcx + this->get_spacingX()));
						}
					}
				}
				HX_STACK_LINE(188)
				xpos = (int)0;
				HX_STACK_LINE(189)
				hx::AddEq(ypos,(bcy + this->get_spacingY()));
			}
		}
	}
return null();
