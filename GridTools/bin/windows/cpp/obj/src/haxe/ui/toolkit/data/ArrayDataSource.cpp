...
Void ArrayDataSource_obj::create( ::Xml config){
{
		...
		HX_STACK_LINE(30)
		delimeter = ::StringTools_obj::replace(delimeter,HX_CSTRING("\\n"),HX_CSTRING("\n"));
		HX_STACK_LINE(32)
		...
		if (((config->firstChild() != null()))){
			HX_STACK_LINE(39)
			nodeText = config->firstChild()->get_nodeValue();
		}
		HX_STACK_LINE(42)
		if (((nodeText != null()))){
			struct _Function_2_1{
...
bool ArrayDataSource_obj::_remove( ){
	HX_STACK_FRAME("haxe.ui.toolkit.data.ArrayDataSource","_remove",0xb8831271,"haxe.ui.toolkit.data.ArrayDataSource._remove","haxe/ui/toolkit/data/ArrayDataSource.hx",81,0x88133302)
	HX_STACK_THIS(this)
	HX_STACK_LINE(81)
	return this->array->__Field(HX_CSTRING("remove"),true)(this->get());
}
