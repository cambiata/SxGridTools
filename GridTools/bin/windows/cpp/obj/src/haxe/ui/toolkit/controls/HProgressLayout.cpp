...
Void HProgressLayout_obj::resizeChildren( ){
{
		...
		if (((value != null()))){
			...
			if (((thumb != null()))){
				HX_STACK_LINE(191)
				hx::SubEq(ucx,thumb->__Field(HX_CSTRING("get_width"),true)());
			}
			HX_STACK_LINE(194)
			...
			HX_STACK_LINE(195)
			Float cx = ((Float(_g1) / Float(((_g2 - scroll->__Field(HX_CSTRING("get_min"),true)())))) * ucx);		HX_STACK_VAR(cx,"cx");
			HX_STACK_LINE(197)
			if (((cx < (int)0))){
				HX_STACK_LINE(198)
				cx = (int)0;
			...
			if (((thumb != null()))){
				HX_STACK_LINE(204)
				hx::AddEq(cx,(Float(thumb->__Field(HX_CSTRING("get_width"),true)()) / Float((int)2)));
			}
			HX_STACK_LINE(207)
			if (((cx == (int)0))){
				HX_STACK_LINE(208)
...
Void HProgressLayout_obj::repositionChildren( ){
{
		...
		if (((background != null()))){
			...
			HX_STACK_LINE(222)
			background->set_y((_g - (Float(background->get_height()) / Float((int)2))));
			HX_STACK_LINE(223)
			value = background->findChild(HX_CSTRING("value"),null(),null());
		}
		HX_STACK_LINE(226)
		...
		if (((value != null()))){
			...
			if (((thumb != null()))){
				HX_STACK_LINE(231)
				Float _g1 = value->__Field(HX_CSTRING("get_x"),true)();		HX_STACK_VAR(_g1,"_g1");
				HX_STACK_LINE(231)
				Float _g2 = (_g1 + value->__Field(HX_CSTRING("get_width"),true)());		HX_STACK_VAR(_g2,"_g2");
				HX_STACK_LINE(231)
				Float xpos = (_g2 - (Float(thumb->__Field(HX_CSTRING("get_width"),true)()) / Float((int)2)));		HX_STACK_VAR(xpos,"xpos");
				HX_STACK_LINE(232)
				thumb->__Field(HX_CSTRING("set_x"),true)(::Std_obj::_int(xpos));
			}
		}
	}
return null();
